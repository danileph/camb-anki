...
const styles = {
  root: {
    base: ({ width, height }: { width: number; height: number }) =>
      css({
        position: "relative",
        width: "100%",
        height,
...
const Select: FC<ISelectProps> = ({
  ...
  useEffect(() => {
    ...
    if (inputElem && inputDimensions?.height === 0) {
      console.log(inputElem.clientHeight);
      setInputDimensions({
  ...
  return (
    <div ref={rootRef} css={styles.root.base(inputDimensions)} {...other}>
      <div
        ...
        onFocus={(e) => {
          if (e.target.classList.contains(inputId)) {
            ...
            !isOpened &&
              document.querySelector<HTMLInputElement>(`.${inputId}`)?.focus();
          }
        }}
        onBlur={(e) => {
          if (e.target.classList.contains(inputId)) {
      ...
      >
        ...
        >
          ...
          {options
            ? options.map((option) => (
                <Option
                  key={`${optionsId}-${option.id}`}
                  id={`${optionsId}-${option.id}`}
                  isChecked={
                    `${optionsId}-${value}` === `${optionsId}-${option.id}`
                  }
                  onClick={() => {
